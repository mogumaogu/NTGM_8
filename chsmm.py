...
import time          ########## 時間計測 ##########
import timeit          ########### 時間計測 ##########
start_time = time.process_time()          ########### 時間計測 ##########
time.sleep(1)          ########### 時間計測 ##########
...
if __name__ == "__main__":
    ...
    def train(epoch):
        ...
        for batch_idx in range(nmini_batches):
            ...
            neglogev = neglogev - log_marg.data.item()
            ...
            if (batch_idx+1) % args.log_interval == 0:
                end_time = time.process_time()          ############################## 時間計測 ##############################
                elapsed_time = end_time - start_time          ############################## 時間計測 ##############################
                print(elapsed_time)          ############################## 時間計測 ##############################
    ...
    def test(epoch):
        ...
        for i in range(len(corpus.valid)):
            ...
            fwd_obs_logps = net.obs_logprobs(Variable(inps, volatile=True), srcenc,
                                             ...
                                             bsz) # L x T x bsz x K
            #srcenc, srcfieldenc, uniqenc = net.encode(Variable(src, torch.no_grad=True),  # bsz x hid
            #                                          Variable(amask, torch.no_grad=True),
            #                                          Variable(uniqfields, torch.no_grad=True))
            #init_logps, trans_logps = net.trans_logprobs(uniqenc, seqlen) # bsz x K, T-1 x bsz x KxK
            #len_logprobs, _ = net.len_logprobs()
            #fwd_obs_logps = net.obs_logprobs(Variable(inps, torch.no_grad=True), srcenc,
            #                                 srcfieldenc, Variable(fmask, torch.no_grad=True),
            #                                 Variable(combotargs, torch.no_grad=True),
            #                                 bsz) # L x T x bsz x K
